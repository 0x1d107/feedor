...
def css_text(sel):
    return lambda h: CSSSelector(sel)(h)[0].text_content() if CSSSelector(sel)(h) else None
def css_html(sel):
    return lambda h:bleach.clean((e[0].text if e[0].text else '')+''.join(tostring(child,encoding='utf-8').decode('utf-8') for child in e[0].iterchildren()),strip=True)if(e:= CSSSelector(sel)(h)) else None
def css_attr(sel,attr):
    return lambda h: CSSSelector(sel)(h)[0].get(attr)
def css_attr_regex(sel,attr,regex,group):
    return lambda h:m[group] if (m:=next(re.finditer(regex, css_attr(sel, attr)(h)),None)) else None
def css_enclosures(sel,attr):
    return lambda h: [FeedParserDict(href=enc.get(attr),type=mimetypes.guess_type(enc.get(attr).split('?')[0])[0], length=0,rel='enclosure') for enc in CSSSelector(sel)(h) if enc.get(attr)]
def css_enclosures_regex(sel,attr,regex,group):
    r = re.compile(regex)
    return lambda h: [FeedParserDict(href=(url:=r.search(enc.get(attr,''))[group]),type=mimetypes.guess_type(url.split('?')[0])[0], length=0,rel='enclosure') for enc in CSSSelector(sel)(h) if enc.get(attr)and r.search(enc.get(attr))]

async def main():
    async with aiohttp.ClientSession() as session:
        print(json.dumps(await HTMLAdapter("https://t.me/s/var_log_shitpost", CSSSelector(".tgme_widget_message"), 
        {
